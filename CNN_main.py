    ...
    def __init__(self):
        ...
        self.layer1 = nn.Sequential(#input 1*28*28
            nn.Conv2d(
                in_channels=1,
                out_channels=6,
                kernel_size=3),  # 6, 26 ,26
            nn.BatchNorm2d(6),
            nn.ReLU(inplace=True))

        self.layer2 = nn.Sequential(
            nn.Conv2d(6, 12, kernel_size=3),  # 12, 24, 24
            nn.BatchNorm2d(12),
            nn.ReLU(inplace=True),
            nn.AvgPool2d(kernel_size=2, stride=2))  # 12, 12,12     (24-2) /2 +1

        self.layer3 = nn.Sequential(
            nn.Conv2d(12, 24, kernel_size=3),  # 24,10,10
            nn.BatchNorm2d(24),
            nn.ReLU(inplace=True))

        self.layer4 = nn.Sequential(
            nn.Conv2d(24, 48, kernel_size=3),  # 48,8,8
            nn.BatchNorm2d(48),
            nn.ReLU(inplace=True),
